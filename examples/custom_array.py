...
mesh2 = ps.createbox(36, 50, 8, sound_a, s)
def compute_array(mesh, src_coord, lis_coord, micarray,num):
    ...
    np.save('/root/data/gsound_rir/rir_' + str(num) + '.npy',res)
    return res
...
def main(num):
    ...
    mic_centerx = random.randint(5,31)
    mic_centery = random.randint(5,31)
    distance1 = round(random.uniform(1.00,1.40),2)
    distance2 = round(random.uniform(1.00,1.40),2)
    ...
    lis_coord = [mic_centerx, mic_centery, 1.7]
